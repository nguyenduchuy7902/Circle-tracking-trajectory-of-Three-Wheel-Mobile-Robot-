...
void subtract(matrix *A, matrix *B) // value return into matrix A
{
    //Check dimensions of all matrix operands
    if ((A->num_rows != B->num_rows) || (A->num_columns != B->num_columns))
        return;
...
//Fill out initial value of matrix
//Answer returned to A
void fill_matrix(matrix *A, unsigned_int8 value) {
    for (int i = 0; i < A->num_rows; i++) {
        for (int j = 0; j < A->num_columns; j++) {
            A->index[i][j] = value;
        }
